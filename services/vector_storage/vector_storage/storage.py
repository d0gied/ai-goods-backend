...
class Storage:
    ...
    def add_by_names(self, names: list[GoodEmbedding]):
        self.name_repo.add(names)
    
    def add_by_images(self, images: list[GoodEmbedding]):
        self.image_repo.add(images)
    
    def add_by_names_images(self, names_images: list[GoodEmbedding]):
        self.name_image_repo.add(names_images)
        
    def delete_by_names(self, ids: list[int]):
        self.name_repo.delete(ids)
    
    def delete_by_images(self, ids: list[int]):
        self.image_repo.delete(ids)
    
    def delete_by_names_images(self, ids: list[int]):
        self.name_image_repo.delete(ids)
    
    def search_by_names(self, name: GoodEmbedding, k: int = 10, threshold: float = None) -> list[int]:
        return self.name_repo.search(name, k, threshold)
    
    def search_by_images(self, image: GoodEmbedding, k: int = 10, threshold: float = None) -> list[int]:
        return self.image_repo.search(image, k, threshold)
    
    def search_by_names_images(self, name_image: GoodEmbedding, k: int = 10, threshold: float = None) -> list[int]:
